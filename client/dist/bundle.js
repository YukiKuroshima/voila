riot.tag2('app', '<nav style="margin-bottom: 0px" class="navbar unique-color" role="navigation"> <div class="container"> <ul class="nav navbar-nav"> <li><a class="navbar-brand" href="#">Voila</a></li> <li><a if="{!localStorage.getItem(\'ticketID\')}" href="#">New Ticket</a></li> <li><a if="{localStorage.getItem(\'ticketID\')}" href="#{localStorage.getItem(\'ticketID\')}/gen">Check in</a></li> <li><a if="{localStorage.getItem(\'ticketID\')}" href="#{localStorage.getItem(\'ticketID\')}/list">Show list</a></li> <li><a if="{localStorage.getItem(\'ticketID\')}" href="#{localStorage.getItem(\'ticketID\')}/post">Test Post</a></li> </ul> </div> </nav> <div id="content-tag"></div> <footer class="page-footer unique-color center-on-small-only"> <div id="footer-tag"></div> </footer>', 'app a,[data-is="app"] a{ color: #9dafc0; }', '', function(opts) {
...
this.saveTicket = e => {
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 201) {
      ...
      _this.successText = 'Success! Please view your ticket by logging in below';
      _this.errorText = '';
      _this.update();
    } else if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 400) {
      ...
      _this.errorText = _this.stringifyError(JSON.parse(xhr.response).message);
      _this.successText = '';
      _this.update();
    } else {
      console.log('else ' + xhr.responseText);
      _this.successText = '';
      _this.alertText = JSON.parse(xhr.response).message;
      _this.update();
    }
  };
...
this.authTicket = e => {
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
      ...
      _this.errorTextAuth = _this.stringifyError(JSON.parse(xhr.response).message);
      _this.successTextAuth = '';
      _this.update();
    } else {
      console.log('Unknown status Response ' + xhr.responseText);
      _this.errorTextAuth = JSON.parse(xhr.response).message;
      _this.successTextAuth = '';
      _this.update();
    }
  };
...
this.postData = e => {
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 201) {
      // Request finished. Do processing here.
      console.log('Response ' + xhr.responseText);
      _this.successText = 'Successfully uploaded data';
      _this.update();
    } else if (xhr.readyState == XMLHttpRequest.DONE) {
      console.log('Response ' + xhr.responseText);
      _this.errorText = 'Something went wrong. Please let your professor know.';
      _this.update();
    }
  };
